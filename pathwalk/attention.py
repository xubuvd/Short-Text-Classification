...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, hidden_size=512,num_heads=4,pad_size=2):
    ...
    def forward(self, X, Y, mask_X, mask_Y):
        ...
        A tuple of two MultiHeadAttention
            ...
            A_Y(X): torch.FloatTensor
                ...
                Shape [batch_size, N, hidden_size]
        """
        pad_mask = X.new_ones((X.size(0), self.pad_size)).long()#[32,2]

        mask_X = torch.cat([pad_mask, mask_X], dim=1)#[32,2+M]
        mask_Y = torch.cat([pad_mask, mask_Y], dim=1)#[32,2+N]
        M_pad, N_pad = mask_X.size(1), mask_Y.size(1)
        mask_X = mask_X[:, None, :, None].repeat(1, self.num_heads, 1, N_pad)#[32,4,2+M,2+N]
        mask_Y = mask_Y[:, None, None, :].repeat(1, self.num_heads, M_pad, 1)#[32,4,2+M,2+N]
...
class AttentionStack(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.co_attns = clones(MultiHeadAttention(hidden_size,num_heads=4,pad_size=2), 3)
        if check_flag(config['model'], 'ca_has_self_attns'):
            self.self_attns = clones(MultiHeadAttention(hidden_size,num_heads=4,pad_size=2), 3)
        ...
        if self.config['model']['ca_has_layer_norm']:
            seself.im_normlf.qe_norm = nn.LayerNorm(hidden_size)
...
class AttentionStackEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        num_cross_attns = config['model']['ca_num_attn_stacks']

        # whether to share the attention weights or not
        if config['model']['ca_has_shared_attns']:
