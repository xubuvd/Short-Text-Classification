...
from Transformer_model import PathWalk_TransformerModel
...
model = PathWalk_TransformerModel(
    config=config["model"],
    node_size=point_size,
    vocabulary=train_dataset.vocabulary,
    num_class=len(class_list),
    device=device,
    padding_size=config['dataset']['max_sequence_length']
).to(device)
...
for epoch in range(start_epoch, config["solver"]["num_epochs"]):
    ...
    for i, batch in tqdm(enumerate(combined_dataloader),total=iterations):
        ...
        loss_total += loss
        loss.backward()
        torch.nn.utils.clip_grad_value_(parameters=model.parameters(),clip_value=2.0)
        optimizer.step()
