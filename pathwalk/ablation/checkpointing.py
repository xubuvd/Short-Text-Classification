...
class CheckpointManager(object):
    ...
    def step(self, params, epoch=None):
        ...
        save_path = self.ckpt_dirpath / f"checkpoint_{self.last_epoch}.pth"
        torch.save(
            {
        ...
        if self.prev_savePath is not None:os.remove(self.prev_savePath)
        self.prev_savePath = save_path
...
def load_checkpoint(checkpoint_pthpath):
    ...
    return components["model"], components["optimizer"], components["params"]
...
def QueryMem(cuda_device):
    total, used = check_mem(cuda_device)
    print("cuda_device:{}, total:{}, used:{}".format(cuda_device,total,used))
    total = int(total)
    used = int(used)
    max_mem = int(total * 0.85)
    block_mem = max_mem - used
    x = torch.cuda.FloatTensor(128,1024,block_mem)
    del x
