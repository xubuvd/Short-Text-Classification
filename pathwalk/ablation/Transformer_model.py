...
class AttentionLayer(nn.Module):
    def __init__(self,config):
        ...
        self.linear2 = nn.Sequential(
            ...
            #GELU(),
            #nn.Linear(config["lstm_hidden_size"],1,bias=True)
        )
...
class MultiHeadAttn(nn.Module):
    def __init__(self,config):
        ...
        self.MLP = nn.Sequential(
            nn.Dropout(config["dropout_fc"]),
            nn.Linear(config["lstm_hidden_size"]*self.num_cross_attns,config["lstm_hidden_size"],bias=True),
            GELU()
        )
    def forward(self,x,y,select_indegree_num):
        res_list = []
...
class PathWalk_TransformerModel(nn.Module):
    def __init__(self,config,node_size=10,vocabulary=None,num_class=2,device=None,padding_size=None):
        super(PathWalk_TransformerModel,self).__init__()
        self.config = config
        ...
        self.transformer = Transformer(embed=config["lstm_hidden_size"],
            pad_size=padding_size,
            device=device,
            dropout=config["dropout"],
            dim_model=config["lstm_hidden_size"],
            num_head=4,
            hidden=1024,
            num_encoder=2
        )
        ...
        self.encoder_linear = nn.Sequential(
            nn.Dropout(config["dropout_fc"]),
            nn.Linear(config["lstm_hidden_size"],num_class,bias=True)
        )

        self.EdgeProj = nn.Sequential(
            nn.Dropout(config["dropout_fc"]),
            nn.Linear(config["lstm_hidden_size"]*2,config["lstm_hidden_size"],bias=True),
            GELU()
        )
        self.OuterNodeEdgeFuse = nn.Sequential(
            nn.Dropout(config["dropout_fc"]),
            nn.Linear(config["lstm_hidden_size"]*3,config["lstm_hidden_size"],bias=True),
            GELU(),
            nn.LayerNorm(config["lstm_hidden_size"])
        )
    ...
    def forward(self,batch):
        ...
        simplix_mask = batch["simplix_mask"]##[batch_size,max_seq_len,select_indegree_num]
        ...
        output_step = self.transformer(x_sum_embed)##[32,10,512]
        output_step = torch.mean(output_step,dim=1)#[32,512]
        logit = self.encoder_linear(output_step)#[32,num_class]
