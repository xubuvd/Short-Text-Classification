...
if __name__ == "__main__":
    ...
    X,Acc = get_data('log_PathWalk_2022052101_58_gpu1_DanMu_300d_select_indegree_num_45.log')
    plt.plot(X,Acc,'-',linewidth = 1.5, color = 'darkred', marker='.', markersize=10, label=r'$\alpha^{(1)}$')

    X,Acc = get_data('log_PathWalk_2022052604_58_gpu1_DanMu_ablation_alpha2.log')
    plt.plot(X,Acc,'-',linewidth = 1.5, color = 'blue', marker='.', markersize=10, label=r'$\alpha^{(2)}$')
    

    X,Acc = get_data('log_PathWalk_2022052602_58_gpu0_DanMu_ablation_alpha3.log')
