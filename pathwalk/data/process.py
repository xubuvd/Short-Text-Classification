...
def load(input_file,encoding='utf-8'):
    #encoding="ISO-8859-1"
    #encoding='utf-8'
	with codecs.open(input_file,'rb',encoding=encoding) as f:data = f.readlines()
	return data
...
def stat(input_file):
    ...
    for line in data:
        ...
        for item in sent:
            item = item.strip()
            if item not in UniqueWordsSet: UniqueWordsSet.append(item)
    ...
    print("uniqueWords:{}".format(len(UniqueWordsSet)))
